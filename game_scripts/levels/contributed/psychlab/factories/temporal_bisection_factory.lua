...
saccade towards/away from a target of that color if the display was long/short.
Simpler animal tasks have designated left=short and right=long,
so this may be slightly harder.
]]
...
function factory.createLevelApi(kwargs)
  ...
  kwargs.numPositions = kwargs.numPositions or NUM_POSITIONS
  kwargs.targetDurations = kwargs.targetDurations or TARGET_DURATIONS
  kwargs.centralTargetLocation = kwargs.centralTargetLocation
      or CENTRAL_TARGET_LOCATION
  ...
  function env:_init(pac, opts)
    ...
    for i = 1, kwargs.numPositions do
      local angle = (i - 1) * math.pi / (kwargs.numPositions)
      local targetPosition0 = targetPosition(angle)
      local targetPosition1 = targetPosition(math.pi + angle)
  ...
  function env:runTrial()
    ...
    local colLongIdx = psychlab_helpers.randomFrom({0, 1})
    local colShortIdx = -colLongIdx + 2
    colLongIdx = colLongIdx + 1
    self._colLong = kwargs.targetColors[colLongIdx]
    self._colShort = kwargs.targetColors[colShortIdx]

    local target = tensor.ByteTensor(self.screenSize.height * kwargs.targetSize,
        self.screenSize.width * kwargs.targetSize, 3):fill(self._colLong)
    self.pac:addWidget{
        name = 'target',
        image = target,
  ...
  function env:selectionPhase()
    ...
    if self._trialDurIdx == 1 then -- short trial
      ...
      self.currentTrial.duration = 0
    elseif self._trialDurIdx == 2 then -- long trial
      targetCorrect = targetLong
      targetIncorrect = targetShort
      self.currentTrial.duration = 1
    end

    -- Regardless of which color is correct, positions are randomized.
    local posCorrectIdx = psychlab_helpers.randomFrom({0, 1})
    local posIncorrectIdx = -posCorrectIdx + 2
    posCorrectIdx = posCorrectIdx + 1

    -- Select the axis of positions for this trial.
    local trialTargetPositions = psychlab_helpers.randomFrom(
        self.targetPositions)
    local posCorrect = trialTargetPositions[posCorrectIdx]
    local posIncorrect = trialTargetPositions[posIncorrectIdx]
    self.currentTrial.correct_pos = posCorrectIdx
    self.currentTrial.incorrect_pos = posIncorrectIdx
