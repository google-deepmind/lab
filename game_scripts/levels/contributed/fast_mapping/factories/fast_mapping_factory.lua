...
function factory.defaultInstructor(key, context, goalGroup)
  local data = {
...
function factory.createTask(kwargs)
  ...
  task.rewardController = factory.createController(
...
function factory.createLevelApi(kwargs)
  ...
  api._levelInstructor = factory.defaultInstructor
  ...
  function api:pickup(id)
    local respawnTime, newInstr, teachObj = self._rewardController:handlePickup(
                                                       self._pickups[id]._desc)
    if newInstr then
      self.setInstruction(newInstr)
    end
    if not teachObj then
      self._pickedUp = true
    end
    return respawnTime
  end
  ...
  function api:nextMap()
    ...
    local instructionGenerator = self._levelInstructor
...
function factory.createController(kwargs, objectCount, distractorObjectCount)
  local controller = {}
  local totalObjectCount = objectCount + distractorObjectCount
  ...
  function controller:handlePickup(description)
    ...
    return -1, newInstruction, shapingObject
  end
