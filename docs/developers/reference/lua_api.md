...
`entity_id` is allowed to be picked up. Return `false` to disallow pickup.
Defaults to `true` if the callback function isn't implemented.

Will not be called if *entity_id* is not a positive integer; this is normally
...
Will not be called if *entity_id* is not an integer; this is normally ensured by
`updateSpawnVars()`.
...
### `lookat`(*entityId*, *lookedAt*, *position*)
